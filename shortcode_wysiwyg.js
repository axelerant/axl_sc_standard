    ...
    insert_form:function (data, settings, instanceId) {
      ...
      tmpdiv.load(aurl, function () {
        ...
        btns[Drupal.t('Insert shortcode')] = function () {
          ...
          dialogdiv.contents().find('.form-item').not(':hidden').find('input,select')
            ...
            .each(function () {
              ...
              if(name.indexOf('sc_content_') == 0) {
                  //allow us to override the selected wysiwyg text if a field called sc_content exists and has a value
                  if (val.length) {
                      data.content = val;
                  }
              } else {
                  if(name.indexOf('sc_') == 0) {
          ...
          shortcode = '[' + shortcode + (options.length ? ' ' + options.join(' ') : '') + ']' + template + data.content + '[/' + shortcode + ']';
        ...
        // TODO: add context there
          $( "input[name^='sc_content_']" ).val( data.content );
          $('.jquery_ui_dialog-dialog').css('top', 100);
